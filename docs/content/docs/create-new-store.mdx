...
<Callout title="Require before loading Handles" type="warn">
    Stores must be required before `Fragment.load` is called. Do this explicitly at the top of `FragmentLoad.client.luau`.
</Callout>
...
require(script.Parent.Stores.PlayerDataStore) -- [!code highlight]
...
require(script.Parent.Stores.PlayerDataStore)
Fragment.load(script.Parent.Handles)

-- When server data arrives, push it into the store
local store = Fragment.useStore("PlayerData")
